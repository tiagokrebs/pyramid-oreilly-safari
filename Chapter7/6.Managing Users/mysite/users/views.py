...
class UserViews:
    ...
    def list(self):
        users = Session.query(User).order_by(User.last_name)
    ...
    def add_handler(self):
        ...
        self.request.session.flash('Added: %s' % user.id)
        url = self.request.route_url('users_list', id=user.id)
    ...
    def edit_handler(self):
        ...
        url = self.request.route_url('users_view', id=self.context.id)
        return HTTPFound(url)
